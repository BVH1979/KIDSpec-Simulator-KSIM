            ...
            data_extractor_TLUSTY, data_extractor_TLUSTY_joint_spec,spec_seeing,SNR_calc_pred_grid, \
                atmospheric_effects,rebinner_with_bins, \
                    R_value,redshifter,order_merge_reg_grid,grid_plotter,SNR_calc_grid,grid_plotter_opp, \
                        model_interpolator,multiprocessing_use
...
from MKID_gaussians import MKID_response,recreator,recreator_one_arm
...
if __name__ == '__main__':
    ...
    if TLUSTY == True:
        ...
        model_spec = data_extractor(object_file,quasar=False,plotting=extra_plots)
    else:
...
if orders_ir[0] != 1:
    ...
    #loading any dead pixels and energy resolution variance
    dead_pixel_list_opt = np.load('DEAD_PIXEL_LISTS/DEAD_PIXELS_OPT.npy')
    dead_pixel_list_ir = np.load('DEAD_PIXEL_LISTS/DEAD_PIXELS_IR.npy')
    number_dead_pixels = len(dead_pixel_list_opt)
    ir_R_Es = np.load('R_E_PIXELS/R_E_PIXELS_IR.npy') 
    opt_R_Es = np.load('R_E_PIXELS/R_E_PIXELS_OPT.npy')
    
    
    #loops through each pixel and applies the order gaussians and saves it as a .npy fil
    print('\n Simulating photon stream.')
     
    os.mkdir('%s/Resample/'%(folder))
    file_path_overlap = '%s/Resample/'%(folder)
    with open('%sIR.txt'%file_path_overlap, 'w') as f:
        f.close()
    with open('%sIR_sky.txt'%file_path_overlap, 'w') as f:
        f.close()
    with open('%sOPT.txt'%file_path_overlap, 'w') as f:
        f.close()
    with open('%sOPT_sky.txt'%file_path_overlap, 'w') as f:
        f.close()
    
    cpus = int(input('Number of CPUs to use: '))
    
    perc_comp = 0
    
    saturated_photons_ir = np.zeros(n_pixels)
    '''
    with h5py.File('PIXEL_PHASE_DATA_SCIENCE.hdf5','w') as hf: #setting up h5 file saving
            print('\n Science MKID array h5 file created.')
            
    with h5py.File('PIXEL_PHASE_DATA_SKY.hdf5','w') as hf:
            print('\n Sky MKID array h5 file created.')
    '''
    if orders_ir[0] != 200:
        jobs = []
        jobs_sky = []
        print('\n Starting multiprocessing of IR arm pixels')
        for i in range(n_pixels):
            jobs.append((pixel_sums_ir[i],order_wavelengths_ir[:,i],orders_ir,ir_R_Es[i],i,file_path_overlap))
            jobs_sky.append((pixel_sums_ir_sky[i],order_wavelengths_ir[:,i],orders_ir,ir_R_Es[i],i,file_path_overlap))
        print('\n Starting multiprocessing of IR arm pixels')
        multiprocessing_use(jobs,cpus,IR=True,sky=False)
        print('\n Starting multiprocessing of IR arm pixels for sky')
        multiprocessing_use(jobs_sky,cpus,IR=True,sky=True)
            
        perc_comp = 0
    
    saturated_photons_opt = np.zeros(n_pixels)
    
    jobs = []
    jobs_sky = []
    for i in range(n_pixels):
        jobs.append((pixel_sums_opt[i],order_wavelengths_opt[:,i],orders_opt,opt_R_Es[i],i,file_path_overlap))
        jobs_sky.append((pixel_sums_opt_sky[i],order_wavelengths_opt[:,i],orders_opt,opt_R_Es[i],i,file_path_overlap))
    
    if __name__ == '__main__':
        print('\n Starting multiprocessing of OPT arm pixels')
        multiprocessing_use(jobs,cpus,IR=False,sky=False)
        print('\n Starting multiprocessing of OPT arm pixels for sky')
        multiprocessing_use(jobs_sky,cpus,IR=False,sky=True)
    
    
    print('100% of pixels complete.')
    ...
    if IR_arm == True:
        ...
        kidspec_raw_output,misidentified_spectrum,percentage_misidentified_pp, \
            percentage_misidentified_tot,no_misident = recreator_one_arm(spec_QE,n_pixels,order_wavelengths,orders_ir,sky=False) #1D spectrum of pixel grid, ORDERS NOT MERGED
        
        kidspec_raw_output_sky,misidentified_sky_spectrum,percentage_sky_misidentified_pp, \
            percentage_sky_misidentified_tot,no_misident_sky = recreator_one_arm(spec_QE,n_pixels,order_wavelengths,orders_ir,sky=True)
    ...
    np.save('Misc/%s_KSIM_OUTPUT_FLUX_SPECTRUM.npy'%object_name,SIM_total_flux_spectrum)
    ...
    residuals_spread = scipy.stats.median_absolute_deviation(res1)*100
    SNR_av = np.median(np.nan_to_num(SNRs[1])[np.nonzero(np.nan_to_num(SNRs[1]))])
    ...
    dead_pixel_opt_amount = len(dead_pixel_list_opt)
    dead_pixel_ir_amount = len(dead_pixel_list_ir)
    ...
    f.write('Flux reduced by factor of %.1f\n'%mag_reduce)
